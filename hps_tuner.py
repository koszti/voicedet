    ...
    if any(indata):
        ...
        for j in range(len(OCTAVE_BANDS) - 1):
            ...
            for i in range(ind_start, ind_end):
                magnitude_spec[i] = (
                    magnitude_spec[i] if magnitude_spec[i] > WHITE_NOISE_THRESH * avg_energy_per_freq else 0
                )

        # interpolate spectrum
        mag_spec_ipol = np.interp(
            np.arange(0, len(magnitude_spec), 1 / NUM_HPS), np.arange(0, len(magnitude_spec)), magnitude_spec
        )
